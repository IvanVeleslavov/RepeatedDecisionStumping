...
# Whitney U Test
using DataFrames, Random, HypothesisTests, Plots, CSV, ZipFile
...
DATA_PATH = normpath(@__DIR__, "../data")

# MURINE RESULTS ////////////////////////////////////////////////////////////
# DATA ENTRY
mur_data = CSV.read(DATA_PATH * "/murine/murine_expression.csv", DataFrame)

# Read in the associated annotation data
mur_labels = CSV.read(DATA_PATH * "/murine/murine_annotations.csv", DataFrame)
...
r = ZipFile.Reader(DATA_PATH*"/xenopus/xenopus_expression.csv.zip")
xen_df = CSV.read(r.files[1], DataFrame)
close(r)

# Read in the associated annotations, adding the Celltype label to the xen_df
xen_labels = CSV.read(DATA_PATH*"/xenopus/xenopus_annotations.csv", DataFrame)
